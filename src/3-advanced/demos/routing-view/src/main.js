...
function homePageView() {
  return div([h1('Welcome to History Examples!'), p(placeholderText())]);
}

function aboutPageView() {
  return div([h1('About me'), p(placeholderText())]);
}

function contactsPageView() {
  return div([h1('Contact me'), p(placeholderText())]);
}
...
function main(sources) {
  const history$ = sources.DOM.select('nav')
    .events('click')
