...
import { h2, div, makeDOMDriver } from '@cycle/dom';
import LabeledSlider from './LabeledSlider';
import { run } from '@cycle/run';

function model(weightSliderValue$, heightSliderValue$) {
  return xs
    .combine(weightSliderValue$, heightSliderValue$)
    .map(([weight, height]) => {
...
function view(bmi$, weightSliderDOM, heightSliderDOM) {
  return xs
    .combine(bmi$, weightSliderDOM, heightSliderDOM)
    .map(([bmi, weightVTree, heightVTree]) =>
      div([weightVTree, heightVTree, h2(`BMI is ${bmi}`)])
    );
}
